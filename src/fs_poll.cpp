...
int uvplus_fs_poll::start(const char *path, unsigned int interval, std::function<void(int status, const uv_stat_t *prev, const uv_stat_t *curr)> fs_poll_callback) {
  auto handle = (uv_fs_poll_t *)context_ptr();
...
void uvplus_fs_poll::fs_poll_cb(uv_fs_poll_t *handle, int status, const uv_stat_t *prev, const uv_stat_t *curr) {
  ...
  if (self->fs_poll_callback) {
    self->fs_poll_callback(status, prev, curr);
  }
}
