...
int uvplus_fs_event::start(const char *path, unsigned int flags, std::function<void(const char *filename, int events, int status)> fs_event_callback) {
  auto event = (uv_fs_event_t *)context_ptr();
...
void uvplus_fs_event::fs_event_cb(uv_fs_event_t *handle, const char *filename, int events, int status) {
  ...
  if (self->fs_event_callback) {
    self->fs_event_callback(filename, events, status);
  }
}
