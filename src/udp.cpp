...
int uvplus_udp::send(const uv_buf_t bufs[], unsigned int nbufs, const struct sockaddr *addr, std::function<void(uvplus_udp *self, int status)> send_callback) {
  auto req = new uv_udp_send_t;
  auto send_callback_ptr = new std::function<void(uvplus_udp *self, int status)>(send_callback);
  auto udp = (uv_udp_t *)context_ptr();
...
int uvplus_udp::recv_start(std::function<void(uvplus_udp *self, size_t suggested_size, uv_buf_t *buf)> alloc_callback,
                           std::function<void(uvplus_udp *self, ssize_t nread, const uv_buf_t *buf, const struct sockaddr *addr, unsigned flags)> recv_callback) {
  auto udp = (uv_udp_t *)context_ptr();
...
void uvplus_udp::send_cb(uv_udp_send_t *req, int status) {
  auto udp = req->handle;
  auto send_callback = static_cast<std::function<void(uvplus_udp *self, int status)> *>(req->data);
  auto handle = static_cast<uvplus_handle *>(udp->data);
  auto self = static_cast<uvplus_udp *>(handle);
  if (*send_callback) {
    (*send_callback)(self, status);
  }
...
void uvplus_udp::recv_cb(uv_udp_t *handle, ssize_t nread, const uv_buf_t *buf, const struct sockaddr *addr, unsigned flags) {
  ...
  if (self->recv_callback) {
    self->recv_callback(self, nread, buf, addr, flags);
  }
}
