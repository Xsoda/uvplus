...
class uvplus_fs_event : public uvplus_handle {
 public:
  ...
  int start(const char *path, unsigned int flags, std::function<void(uvplus_fs_event *self, const char *filename, int events, int status)> fs_event_callback);
  int stop();
  int getpath(char *buffer, size_t *size);

 private:
  std::function<void(uvplus_fs_event *self, const char *filename, int events, int status)> fs_event_callback;
