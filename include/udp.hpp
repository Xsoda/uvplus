...
public:
  ...
  int set_ttl(int ttl);
  int send(const uv_buf_t bufs[], unsigned int nbufs, const struct sockaddr *addr, std::function<void(uvplus_udp *self, int status)> send_callback);
  int try_send(const uv_buf_t bufs[], unsigned int nbufs, const struct sockaddr *addr);
  int recv_start(std::function<void(uvplus_udp *self, size_t suggested_size, uv_buf_t *buf)> alloc_callback,
  ...
  int recv_stop();
private:
  std::function<void(uvplus_udp *self, size_t suggested_size, uv_buf_t *buf)> alloc_callback;
  std::function<void(uvplus_udp *self, ssize_t nread, const uv_buf_t *buf, const struct sockaddr *addr, unsigned flags)> recv_callback;
