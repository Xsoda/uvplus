...
public:
  ...
  int start(int events, std::function<void(uvplus_poll *self, int status, int events)> poll_callback);
  int stop();
private:
  std::function<void(uvplus_poll *self, int status, int events)> poll_callback;
