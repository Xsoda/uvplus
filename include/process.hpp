...
public:
  ...
  void set_exit_callback(std::function<void(uvplus_process *self, int64_t exit_status, int term_signal)> exit_callback);
  int kill(int signum);
...
private:
  ...
  std::function<void(uvplus_process *self, int64_t exit_status, int term_signal)> exit_callback;
